...
void drawNPCs(){
  ...
  angleNPC1 = atan(abs(player.position.y-yNPC)/abs(player.position.x-xNPC));
  ...
  xNPC = ((1-alpha) * xNPC + alpha * (player.position.x - signum(player.position.x-xNPC)* 80 *cos(angleNPC1)));
  yNPC = ((1-alpha) * yNPC + alpha * (player.position.y - signum(player.position.y-yNPC)* 80 *sin(angleNPC1)));
